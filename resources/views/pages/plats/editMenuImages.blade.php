...
<!--Pages de modification images plats-->

@extends(( auth()->user()->role == 'customer' ) ? 'layouts.app' : 'layouts.backend' )

@section('title', config('app.name'). ' - edition image plats')
...
<form method='post' action="{{ route('updateMenuImages') }}" enctype="multipart/form-data" class="px-4 py-3" style="min-width: 350px">
    ...
    <div class="mb-2">
        <label for="formFile" class="form-label">ajouter image plat(png, jpg, jpeg) Taille max: 10MB</label>
    ...
    <div class="row">
        <div>
            <button type="submit" class="btn btn-outline-success">Valider le changement</button>
